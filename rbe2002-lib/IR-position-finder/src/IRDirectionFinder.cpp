...
uint32_t stopTimeMQTT = 0;

  void IRDirectionFinder::setTimer(){
    stopTimeMQTT = millis() + 1000;
  }

  bool IRDirectionFinder::checkTimerExpired()
{
  bool TimerExpired = false;
  uint32_t currTime = millis();

  if (currTime >= stopTimeMQTT)
  {
    TimerExpired = true;
...
void IRDirectionFinder::handleTimerExpired(String printString){
