...
float LSM6::calculateChangeInPitchAngle(float rate, float gyValue, float previousBias){
  float sensitivity = 0.0175;
  float period = 1.0/rate;
  float changeInPitchAngle = period * sensitivity * (gyValue - previousBias);

  return changeInPitchAngle;
}
