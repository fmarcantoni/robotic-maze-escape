...
bool MaxBotixPulse::getDistance(float& distance)
{
    ...
    if(state & ECHO_RECD)
    {
        ...
        distance = (echoLength)/(147); // TODO: Edit to calculate distance
        newReading = true;
    }
...
*/
const uint32_t ADC_INTERVAL = 50; // TODO: Edit to a reasonable time
...
bool MaxBotixAnalog::getDistance(float& distance)
{
    ...
    {
        lastPing = currTime;
        uint16_t adcResult = analogRead(22);
        distance = ((adcResult/1023.0)*5.0)/(5.0/512.0); // TODO: Edit to calculate distance
